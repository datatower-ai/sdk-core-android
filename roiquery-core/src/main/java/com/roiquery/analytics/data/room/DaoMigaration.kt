...
val MIGRATION_2_3 = object : Migration(2,3){
    ...
    override fun migrate(database
        ...
        database.execSQL("DROP TABLE `configs`")
        database.execSQL("ALTER TABLE `_new_configs` RENAME TO `configs`")
        database.execSQL("CREATE TABLE IF NOT EXISTS `_new_events` (`_id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL," +
        ...
        try {
            database.let {
                ...
                while (cursor.moveToNext()) {
                    val data = cursor.getString(cursor.getColumnIndex(DataParams.KEY_DATA))
                    val id = cursor.getString(cursor.getColumnIndex("_id"))
                    val eventName = JSONObject(data).optJSONObject(Constant.EVENT_BODY)?.let { eventBody->
